...
'
'Clase para Abrir el Formulario del Sistema
'
'
'''''''''''''''''''''''''''''''''''''''''''
Option Explicit
...
Open variable For Input As 1
 Do While Not EOF(1)
  ...
  'Datos ----------------------------------------------
       Line Input #1, Nombre
                      xnombre = ModGuardar.es.desescriptar(Nombre)
       Line Input #1, Nombre2
                      xnombre2 = ModGuardar.es.desescriptar(Nombre2)
       Line Input #1, apellido
                      xapellido = ModGuardar.es.desescriptar(apellido)
       Line Input #1, apellido2
                      xapellido2 = ModGuardar.es.desescriptar(apellido2)
       Line Input #1, direccion
                      xdireccion = ModGuardar.es.desescriptar(direccion)
       Line Input #1, direccion2
                      xdireccion2 = ModGuardar.es.desescriptar(direccion2)
       Line Input #1, localidad
                      xlocalidad = ModGuardar.es.desescriptar(localidad)
       Line Input #1, Pais
                      xPais = ModGuardar.es.desescriptar(Pais)
       Line Input #1, telefono
                      xtelefono = ModGuardar.es.desescriptar(telefono)
       Line Input #1, cel
                      xcel = ModGuardar.es.desescriptar(cel)
       Line Input #1, email
                      xemail = ModGuardar.es.desescriptar(email)
       Line Input #1, facebook
                      xfacebook = ModGuardar.es.desescriptar(facebook)
       Line Input #1, comentario_general
                      xcomentario_general = ModGuardar.es.desescriptar(comentario_general)
  'Evento --------------------------------------------
  
       Line Input #1, hora
                      xhora = ModGuardar.es.desescriptar(hora)
                      frmprograma.listado(0).AddItem xhora
       Line Input #1, Tipo
                      xtipo = ModGuardar.es.desescriptar(Tipo)
                      frmprograma.listado(1).AddItem xtipo
       Line Input #1, intervalo
                      xintervalo = ModGuardar.es.desescriptar(intervalo)
                      frmprograma.listado(2).AddItem xintervalo
       Line Input #1, comentario
                      xcomentario = ModGuardar.es.desescriptar(comentario)
                      frmprograma.listado(3).AddItem xcomentario
       Line Input #1, Filtro
                      xfiltro = ModGuardar.es.desescriptar(Filtro)
                      frmprograma.Filtro.AddItem xfiltro
  'Dias ----------------------------------------------
       Line Input #1, lunes
                      xlunes = ModGuardar.es.desescriptar(lunes)
                      frmprograma.lunes(0).AddItem xlunes
       Line Input #1, martes
                      xmartes = ModGuardar.es.desescriptar(martes)
                      frmprograma.martes.AddItem xmartes
       Line Input #1, miercoles
                      xmiercoles = ModGuardar.es.desescriptar(miercoles)
                      frmprograma.miercoles.AddItem xmiercoles
       Line Input #1, jueves
                      xjueves = ModGuardar.es.desescriptar(jueves)
                      frmprograma.jueves.AddItem xjueves
       Line Input #1, viernes
                      xviernes = ModGuardar.es.desescriptar(viernes)
                      frmprograma.viernes.AddItem xviernes
       Line Input #1, sabado
                      xsabado = ModGuardar.es.desescriptar(sabado)
                      frmprograma.sabado.AddItem xsabado
       Line Input #1, domingo
                      xdomingo = ModGuardar.es.desescriptar(domingo)
                      frmprograma.domingo.AddItem xdomingo

  'Puerto --------------------------------------------
       Line Input #1, p1
                      pu1 = ModGuardar.es.desescriptar(p1)
       Line Input #1, p2
                      pu2 = ModGuardar.es.desescriptar(p2)
       Line Input #1, p3
                      pu3 = ModGuardar.es.desescriptar(p3)
       Line Input #1, p4
                      pu4 = ModGuardar.es.desescriptar(p4)
       Line Input #1, p5
                      pu5 = ModGuardar.es.desescriptar(p5)
       Line Input #1, p6
                      pu6 = ModGuardar.es.desescriptar(p6)
       Line Input #1, p7
                      pu7 = ModGuardar.es.desescriptar(p7)
       Line Input #1, p8
                      pu8 = ModGuardar.es.desescriptar(p8)
 'apagado ----------------------------------------------------
       Line Input #1, commando
                      xcommando = ModGuardar.es.desescriptar(commando)
                      frmprograma.liscomando.AddItem (xcommando)
       Line Input #1, comentario1
                      xcomentario1 = ModGuardar.es.desescriptar(comentario1)
                      frmprograma.lisdialogo.AddItem (xcomentario1)
       Line Input #1, tiempo
                      xtiempo = ModGuardar.es.desescriptar(tiempo)
